...
const PROJECT_COLS: &str = "id, company_id, name, description, status, created_at, updated_at";
...
pub async fn update_project(
    ...
    let q = format!("UPDATE projects SET name = COALESCE($2, name), description = COALESCE($3, description), status = COALESCE($4, status), updated_at = now() WHERE id = $1 RETURNING {}", PROJECT_COLS);
    sqlx::query_as::<_, Project>(&q)
        .bind(id)
        .bind(&input.name)
        .bind(&input.description)
